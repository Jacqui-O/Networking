...
//-----------------------------End of Connect to Client--------------------------------------------------//
     ...
     /*  Reading 1st struct sent from client
         memcpy buffer to struct  
     */
     bzero(recvBuff,5000);
...
//receiving only 1 byte at a time, not writing here
    //next todo is to write to client, not read


    /* 2nd Read Request Close */
    //suppose to be read here?
    //
    bzero(recvBuff, 5000);
    n = read(connfd,recvBuff,4999);
    memcpy(&tcp, recvBuff, 4999);
...
void recvFile(int connfd)//pass struct as buffer?
{
